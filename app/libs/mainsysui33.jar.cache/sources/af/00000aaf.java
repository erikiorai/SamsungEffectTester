...
/* loaded from: mainsysui33.jar:com/airbnb/lottie/network/NetworkFetcher.class */
public class NetworkFetcher {
    ...
    public final LottieResult<LottieComposition> getResultFromConnection(HttpURLConnection httpURLConnection) throws IOException {
        ...
        if (str.contains("application/zip")) {
            ...
            fromJsonInputStreamSync = LottieCompositionFactory.fromZipStreamSync(new ZipInputStream(new FileInputStream(this.networkCache.writeTempCacheFile(httpURLConnection.getInputStream(), fileExtension2))), this.url);
            fileExtension = fileExtension2;
        } else {
            Logger.debug("Received json response.");
            FileExtension fileExtension3 = FileExtension.JSON;
            fromJsonInputStreamSync = LottieCompositionFactory.fromJsonInputStreamSync(new FileInputStream(new File(this.networkCache.writeTempCacheFile(httpURLConnection.getInputStream(), fileExtension3).getAbsolutePath())), this.url);
